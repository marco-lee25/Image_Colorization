...
class Discriminator(nn.Module):
    ...
    def get_layers(self, ni, nf, kernel_size=self.Config.kernel_size, stride=self.Config.stride, padding=self.Config.padding, norm=True, activation=True): 
        layers = [nn.Conv2d(ni, nf, kernel_size, stride, padding, bias=not norm)]          
        if norm: 
            layers += [nn.BatchNorm2d(nf)]
        if activation:
            layers += [nn.LeakyReLU(self.Config.LeakyReLU_slope, True)]
