...
from .GAN import build_backbone_unet, MainModel, pretrain_generator
import torch
...
class model_trainer():
    def __init__(self, train_loader, ic, oc, config, display=100):
        ...
        self.loss = nn.L1Loss()        
        pretrain_generator(self.generator, train_loader, self.opt, self.loss, config.epochs)
        ...
        self.model = MainModel(config, generator=self.generator).to(device)

    def pretrain_generator(sel, generator, train_dl, opt, criterion, epochs):
    '''
